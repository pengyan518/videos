        ...
        <div className="grid lg:grid-cols-[1.53fr_1fr] lg:gap-6">
          <div className="w-full pb-4 lg:pb-0">
            <figure className="relative" ref={hoverRef}>
              <ThumbWrapper item={hero} sectionName={sectionName} categoryName={keyName}>
                {(myItem: any) => (
                  <>
                    <ThumbView item={myItem} showLargeThumb showIcon parentHover={isHover} />
                    <div className={`w-full flex absolute bottom-0 h-[8rem] md:h-[9rem] px-3 py-2 md:p-4 items-end text-white`}>
                      <Gradient
          ...
          <div className="grid grid-rows-3 gap-3">
            {thumbs
              .filter(item => !!item)
              .map(item => {
                const {descriptionLong, description, title: itemTitle, textShort} = item
                return (
                  <div key={item.id}>
                    <ThumbWrapper
                      item={item}
                        ...
                        <>
                          ...
                          <div>
                            ...
                            <div
                              ...
                              dangerouslySetInnerHTML={{__html: description || descriptionLong || `“${textShort}”`}}
                            />
                          </div>
                        </>
                      )}
